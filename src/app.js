...
function checkIsUuid(request, response, next){
  ...
  if(!isUuid(id)){
    return response.status(400).json({error: "This ID does not match"});
  }
  return next();
...
app.put("/repositories/:id",checkIsUuid, (request, response) => {
  const {id} = request.params;
  const {title, url, techs} = request.body;
...
app.delete("/repositories/:id", checkIsUuid, (request, response) => {
  const {id} = request.params;
